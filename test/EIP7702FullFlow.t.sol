...
contract EIP7702FullFlowTest is Test {
    ...
    function test_EIP712_Full_Lifecycle() public {
        ...
        bytes32 domainSeparator = keccak256(abi.encode(
            ...
            keccak256(bytes("EIP7702Proxy")), keccak256(bytes("1")), block.chainid, eoa
        ));
        bytes32 structHash = keccak256(abi.encode(
            keccak256("EIP7702ProxyImplementationSet(uint256 chainId,address proxy,uint256 nonce,address currentImplementation,address newImplementation,bytes32 callDataHash,address validator,uint256 expiry)"),
            block.chainid, address(proxy), 0, address(0), address(impl), keccak256(""), address(0), expiry
        ));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        (uint8 v1, bytes32 r1, bytes32 s1) = vm.sign(eoaKey, digest);

        vm.prank(sponsor);
        EIP7702Proxy(payable(eoa)).setImplementation(address(impl), "", address(0), expiry, abi.encodePacked(r1, s1, v1));

        BatchImplementation.Call[] memory calls = new BatchImplementation.Call[](1);
        calls[0] = BatchImplementation.Call({
            to: address(nft), value: 0,
            data: abi.encodeWithSelector(nft.transferFrom.selector, eoa, address(0x123), 7702)
        });

        bytes32[] memory callHashes = new bytes32[](1);
        callHashes[0] = keccak256(abi.encode(keccak256("Call(address to,uint256 value,bytes data)"), calls[0].to, 0, keccak256(calls[0].data)));
        bytes32 batchStructHash = keccak256(abi.encode(
            keccak256("Batch(uint256 nonce,Call[] calls)Call(address to,uint256 value,bytes data)"),
            0, keccak256(abi.encodePacked(callHashes))
        ));
        bytes32 batchDomain = keccak256(abi.encode(
            keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
            keccak256(bytes("BatchAccount")), keccak256(bytes("1")), block.chainid, eoa
        ));
